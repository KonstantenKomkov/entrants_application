...
achievements = dictionaries['mag_achievements']
...
competition_groups_and_disciplines = db((db.a_competition_groups.FO != 4) & (db.t_levels.LEVEL_GROUP == 2)).select(
...
all_cg = cache.ram('all_mag_cg', lambda: bak_and_mag.get_all_cg(2), time_expire=60 * 60 * 24)
    ...
    def decorated():
        ...
        for x in information_list:
            if session[x] is None:
                redirect(URL('mag', 'swed', vars=dict(session=x)))
        else:
            return fun()
    return decorated
...
@email_in_session
def select_cg():
    my_data = cache.ram('mag_cg', lambda: bak_and_mag.create_json(all_cg), time_expire=60 * 60 * 24)
    ...
    if session.prog_list is not None:
        ...
        for a in session.prog_list:
            ...
            for cg in competition_groups_and_disciplines:
                if cg.a_competition_groups.id == int(a['cg_id']):
                    if [cg.t_disciplines.NAME, cg.t_disciplines.MIN_SCORE] not in cg_subjects:
                        cg_subjects.append([cg.t_disciplines.NAME, cg.t_disciplines.MIN_SCORE])

            for d in cg_subjects:
                disc = f'{disc}["{d[0]}","{d[1]}"],'
            disc = disc[0:-1]
            ...
            for cg in all_cg:
                if cg['CG_ID'] == int(a['cg_id']):
                    ...
                    if (session.edu_lvl == '5') or (session.edu_lvl == '6') or \
    ...
    if (educational_fo is not None) and (faculty is not None) and (direction is not None) and (program is not None):
        for c_g in all_cg:
            if (educations_form[c_g['FO']] == educational_fo) and (faculties[c_g['FAC']] == faculty) and \
                ...
                cg_subjects = list()
                for cg in competition_groups_and_disciplines:
                    if cg.a_competition_groups.id == cg_id:
                        if [cg.t_disciplines.NAME, cg.t_disciplines.MIN_SCORE] not in cg_subjects:
                            cg_subjects.append([cg.t_disciplines.NAME, cg.t_disciplines.MIN_SCORE])
                for d in cg_subjects:
                    disc = f'{disc}["{d[0]}","{d[1]}"],'
                ...
                if (session.edu_lvl != '5') and (session.edu_lvl != '6'):
...
def marks():
    ...
    temp_subject = -1
    index = 1
    for x in entrant_cg_and_disciplines:
        if index < len(max_cg) + 1:
            if x.t_disciplines.id != temp_subject:
                current_test_datetime = list()
                for y in entrant_cg_and_disciplines:
                    if y.a_competition_groups.id == x.a_competition_groups.id:
                        if y.t_exams_schedule.EXAM_DATE is not None:
                            if y.t_exams_schedule.EXAM_DATE > datetime.datetime.now().date():
                                current_test_datetime.append(
                                    OPTION(f'{y.t_exams_schedule.EXAM_DATE.strftime("%dd.%mm.%YY")} '
                                           f'{y.t_exams_schedule.EXAM_TIME.strftime("%H:%M")}'))
                tr.append(DIV(DIV(LABEL(x.t_disciplines.NAME, _for=f'exam_date_{index}'),
                                  SELECT(current_test_datetime, _name=f'exam_date_{index}', _id=f'exam_date_{index}',
                                         ...
                                         _class='form-control'),
                                  INPUT(_type='hidden', _name=f'exam_name_{index}', _value=x.t_disciplines.NAME),
                                  _class='col'), _class='form-group row'))
                temp_subject = x.t_disciplines.id
                index += 1
        else:
            break
    ...
    form = FORM(DIV(DIV(P(f'{T("You need to pass the entrance tests in the subjects")}:', _class='paragraf'),
                ...
                DIV(tr),
                DIV(DIV(INPUT(_type='submit', _value=T('Next'), _class='good-btn my-3 w-100', _id='next'), _class='col'),
    ...
    return DIV(DIV(DIV(DIV(_class='col-6'),
                   ...
                   DIV(DIV(
                       A(DIV(XML('<span class="glyphicon glyphicon-pencil"></span><p class="p-btn">Редактировать</p>'),
                             _class='my-btn-conteiner'), _href=URL('mag', 'swed'),
                   ...
                   DIV(DIV(P('Прошу допустить меня к участию в конкурсе для поступления в магистратуру на:',
                             _class='paragraf statement'), _class='col'), _class='row'),
                   DIV(DIV(XML(competition), _class='col'), _class='row'),
                   DIV(DIV(
                       A(DIV(XML('<span class="glyphicon glyphicon-pencil"></span><p class="p-btn">Редактировать</p>'),
                             _class='my-btn-conteiner'), _href=URL('mag', 'select_cg'),
                         _class='good-btn col-sm-12 btn-st-personal hover-none'), _class='col-lg-4 col-12'),
                       _class='row mb-2'),
                   ...
                   DIV(DIV(
                       A(DIV(XML('<span class="glyphicon glyphicon-pencil"></span><p class="p-btn">Редактировать</p>'),
                             _class='my-btn-conteiner'), _href=URL('mag', 'marks'),
                         _class='good-btn col-sm-12 btn-st-personal hover-none'), _class='col-lg-4 col-12'),
                       _class='row mb-2'),
                   DIV(DIV(XML(education_xml), _class='col'), _class='row'),
                   DIV(DIV(
                       A(DIV(XML('<span class="glyphicon glyphicon-pencil"></span><p class="p-btn">Редактировать</p>'),
                             _class='my-btn-conteiner'), _href=URL('mag', 'edu'),
                         _class='good-btn col-sm-12 btn-st-personal hover-none'), _class='col-lg-4 col-12'),
                       _class='row mb-2'),
                   ...
                   DIV(DIV(
                       A(DIV(XML('<span class="glyphicon glyphicon-pencil"></span><p class="p-btn">Редактировать</p>'),
                             _class='my-btn-conteiner'), _href=URL('mag', 'docs'),
                         _class='good-btn col-sm-12 btn-st-personal hover-none'), _class='col-lg-4 col-12'),
                       _class='row mb-2'),
                   _class='conteiner'))
...
def send_app():
    ...
    form = bak_and_mag.agreement_form(1, request.now.year)
    if form.process(formname='form').accepted:
        try:
